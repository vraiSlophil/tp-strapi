...
<template>
  <article class="card rounded-[2rem] border border-slate-200/70 bg-white/90 shadow-sm backdrop-blur">
    <figure v-if="article.cover" class="overflow-hidden border-b border-slate-200/70 bg-slate-100">
      <img :src="article.cover.url" :alt="article.cover.alternativeText || article.title"
    ...
    <div class="card-body gap-5">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="space-y-3">
          <div class="flex flex-wrap gap-2">
            <span v-for="category in article.categories" :key="category.slug || category.name"
        ...
        <div
          class="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600">
          <span class="material-symbols-rounded text-sky-700">rss_feed</span>
          <span>Strapi</span>
        </div>
      </div>

      <div class="rounded-3xl border border-slate-200/80 bg-slate-50/90 p-4">
      ...
      <div class="card-actions justify-end">
        <NuxtLink v-if="articleHref" :to="articleHref" class="btn btn-primary rounded-full px-5">
          Lire l’article
        </NuxtLink>
        <button v-else class="btn btn-disabled rounded-full px-5" type="button">
